...
The highlight-pipeline source of truth is `Documentation/STREAMCUT_ARCHITECTURE_BRIEF.md`; `Documentation/clip-quality-plan.md` holds the leverage ladder and remaining hook/subtitle/resize work. Order is by leverage (moment-selection is the brain; the rest is plumbing), but ground-truth (`TASK-087`) runs first because it gates honest detector tuning. Decisions locked: local Qwen on the GPU worker; word-by-word karaoke subtitles; persist the word timings whisper already computes. Highlight-architecture note (supersedes the earlier "hybrid detection" framing): detection and judgment are two distinct layers — the LLM detector walks the transcript for **recall** ("is something here?", floods a candidate pool), and selection is an **arithmetic fusion** of orthogonal voters (`text` worthiness + `audio` + `chat` density, additive ranks, rank-all, never confidence-sorted or cut-to-N). The LLM is never a ranker/selector. The current floor is shipped on `develop` (commit `93c73cc`): temporal NMS (w=45s, dedup-for-display) replaced the old top-N-by-score cut; stage-2 LLM select-to-N is unused and `hybrid.py` is env-gated off.
